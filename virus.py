...
class Virus():
    def __init__(self, params):
        ...
        self.r0 = params["r0"]
        if params["percent_mild"] < 0:
            raise ValueError("percent_mild cannot be negative!")
        self.percent_mild = params["percent_mild"]
        if params["percent_severe"] < 0:
            raise ValueError("percent_severe cannot be negative!")
        self.percent_severe = params["percent_severe"]
        if params["fatality_rate"] < 0:
            raise ValueError("fatality_rate cannot be negative!")
        self.fatality_rate = params["fatality_rate"]
        if params["serial_interval"] < 0:
            raise ValueError("serial_interval cannot be negative!")
